  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col" role="dialog" aria-label="Player penuh">
      ...
      <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-10 px-6 pb-4 overflow-y-auto">
        <div className="flex flex-col items-center gap-5 lg:w-1/2">
          {c.artwork ? (
            ...
            <img src={`/api/artwork/${c.artwork}`} alt={`Sampul ${c.album}`} className="h-64 w-64 sm:h-80 sm:w-80 rounded-xl object-cover shadow-2xl shadow-black/50" />
          ) : (
      ...
      <div className="border-t border-border px-6 py-4 flex items-center gap-4">
        ...
        <button onClick={p.toggle} aria-label={p.isPlaying ? 'Jeda' : 'Putar'} className="rounded-full bg-accent text-on-accent p-4 hover:bg-accent-hover hover:scale-105 transition-all cursor-pointer">
          {p.isPlaying
        ...
        <div className="flex-1 flex items-center gap-2 text-xs text-muted tabular-nums">
          <span>{fmtDuration(p.currentTime * 1000)}</span>
          <input type="range" min={0} max={p.duration || 0} step={0.1} value={p.currentTime}
            onChange={(e) => p.seek(Number(e.target.value))}
            className="flex-1" aria-label="Posisi lagu" />
          <span>{fmtDuration(p.duration * 1000)}</span>
        </div>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" className="text-muted" aria-hidden="true">
